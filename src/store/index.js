...
export default createStore({
  ...
  actions: {
    ...
    async registerUser({ commit }, payload) {
      try {
        const response = await fetch(`${process.env.VUE_APP_MARKET_PLACE_APP_API_URL}/users`, {
          method: 'POST',
          headers: {
    ...
    async loginUser({ commit }, payload) {
      try {
        const response = await fetch(`${process.env.VUE_APP_MARKET_PLACE_APP_API_URL}/tokens`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
